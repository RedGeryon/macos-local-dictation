...
child process. The NVIDIA GGUF model is separately downloaded and loaded once at
application startup. Quitting terminates the child and releases model memory.
...
- `session.update`: 16 kHz English configuration; conversation streams request
  800 ms endpointing and word timestamps.
